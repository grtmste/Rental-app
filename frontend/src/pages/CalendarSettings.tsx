...
export default function CalendarSettings() {
  ...
  async function syncAll() {
    ...
    setSyncResult(null)
    try {
      const res = await api.post('/calendar-sync/sync-all')
      setSyncResult(res.data)
    } catch (err: any) {
      setSyncResult({ ok: false, message: err?.response?.data?.error || err?.message || 'Tundmatu viga' })
    } finally {
      setSyncing(false)
    }
  }
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
        ...
        <div className="border-t border-gray-100 pt-4">
          <div className="flex items-center justify-between">
            <div>
              <div className="text-sm font-medium text-gray-700">Sünkrooni kõik projektid</div>
              <div className="text-xs text-gray-500 mt-0.5">Lisab kõik olemasolevad projektid Google Calendari</div>
            </div>
            <button
              onClick={syncAll}
              disabled={syncing}
            ...
            >
              {syncing ? 'Sünkroonimine...' : 'Sünkrooni kõik projektid'}
            </button>
          </div>
          {syncResult && (
            <div className={`mt-3 p-3 rounded-lg text-sm ${syncResult.ok ? 'bg-green-50 text-green-800 border border-green-200' : 'bg-red-50 text-red-800 border border-red-200'}`}>
              <span className="font-semibold">{syncResult.ok ? '✓ ' : '✗ '}</span>{syncResult.message}
            </div>
          )}
        </div>
      </div>
