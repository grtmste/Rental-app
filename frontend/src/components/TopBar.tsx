...
export default function TopBar({ onMenuClick }: TopBarProps) {
  ...
  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault()
    if (searchQuery.trim()) navigate(`/app/equipment?search=${encodeURIComponent(searchQuery.trim())}`)
  }

  const roleLabel: Record<string, string> = { admin: 'Admin', manager: 'Haldur', crew: 'Meeskond' }
    ...
    <header className="h-16 bg-white border-b border-gray-200 flex items-center px-4 gap-4 no-print">
      ...
      <form onSubmit={handleSearch} className="flex-1 max-w-lg">
        <div className="relative">
          ...
          <input
            type="text"
            placeholder="Otsi seadmeid, projekte..."
            value={searchQuery}
      ...
      <div className="flex items-center gap-3 ml-auto">
        ...
        <div className="relative" ref={dropdownRef}>
          <button onClick={() => setDropdownOpen(!dropdownOpen)} className="flex items-center gap-2 p-1 rounded-lg hover:bg-gray-100 transition-colors">
            <div className="h-8 w-8 rounded-full bg-primary flex items-center justify-center">
              <span className="text-white text-sm font-medium">{user?.name?.charAt(0)?.toUpperCase() || 'K'}</span>
            </div>
            <div className="hidden sm:block text-left">
              <p className="text-sm font-medium text-gray-900">{user?.name || 'Kasutaja'}</p>
